...
import { getHoldingProfile, updateHoldingProfile } from '../api/holdings'
import { getApiErrorMessage } from '../api/client'
import type { HoldingProfile, AvailableSource } from '../types'
...
export default function HoldingProfilePage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const [form] = Form.useForm()

  const [profile, setProfile] = useState<HoldingProfile | null>(null)
  ...
  const holdingId = id ? parseInt(id) : 0

  const loadProfile = useCallback(async () => {
    if (!holdingId) return
    setPageState('loading')
    try {
      const data = await getHoldingProfile(holdingId)
      setProfile(data)
  ...
  const handleSaveSettings = async () => {
    try {
      ...
      setSaving(true)
      const updated = await updateHoldingProfile(holdingId, {
        name: values.name,
        inn: values.inn || null,
  ...
  if (pageState === 'loading') {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', padding: '80px 0' }}>
        <Spin size="large" tip="Загрузка профиля холдинга..." />
      </div>
    )
  }
          ...
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            {availableSources.length === 0 && (
              <Alert
                ...
                description="У вашего холдинга пока нет настроенных источников законодательства."
                type="info"
                showIcon
              />
            )}
  ...
  return (
    <div style={{ padding: 24 }}>
      <Breadcrumb
        items={[
          { title: <span onClick={() => navigate('/')} style={{ cursor: 'pointer' }}>Главная</span> },
          { title: profile ? `${profile.legal_form} ${profile.name}` : 'Профиль холдинга' },
        ]}
        style={{ marginBottom: 16 }}
      />
      ...
      <Title level={3} style={{ marginBottom: 4 }}>
        {profile ? `${profile.legal_form} "${profile.name}"` : 'Профиль холдинга'}
      </Title>
      {profile?.inn && (
