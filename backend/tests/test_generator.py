...
@pytest_asyncio.fixture
async def company_with_all_fields(test_session: AsyncSession) -> Company:
    """Create a company with all generation fields set."""
    company = Company(
        name="Gost Company",
        inn="7701123456",
        legal_form="ООО",
...
@pytest_asyncio.fixture
async def admin_user_with_company(test_session: AsyncSession, company_with_all_fields: Company) -> User:
    """Create an admin user with active company."""
    user = User(email="admin@gost.ru", is_verified=True)
    test_session.add(user)
    await test_session.flush()
    uc = UserCompany(user_id=user.id, company_id=company_with_all_fields.id, role="admin")
    test_session.add(uc)
    user.active_company_id = company_with_all_fields.id
    await test_session.flush()
    return user


@pytest.fixture
def admin_token(admin_user_with_company: User) -> str:
    """Generate an access token for the admin user."""
    return create_access_token(admin_user_with_company.id)


# ─── Test 1: Mock mode generation ─────────────────────────────────────────
# Test removed: relied on deleted GigaChatClient


# ─── Test 2: Mock generation via API endpoint ──────────────────────────────

@pytest.mark.asyncio
@pytest.mark.skip(reason="Requires generator_service override for test env")
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Requires generator_service override for test env")
async def test_generate_with_drafts(test_session: AsyncSession, admin_user_with_company: User):
...
@pytest.mark.asyncio
@pytest.mark.skip(reason="Requires generator_service override for test env")
async def test_generate_with_influencing_docs(test_session: AsyncSession, admin_user_with_company: User):
    ...
    influencing_doc = Document(
        ...
        status="approved",
        created_by=admin_user_with_company.id,
    )
...
@pytest.mark.asyncio
async def test_prompt_builder(company_with_all_fields: Company):
    """Test that PromptBuilder generates correct system and user messages."""
    messages = prompt_builder.build_messages(
        company=company_with_all_fields,
    ...
    # System prompt should contain company info
    assert "Gost Company" in system_content
...
async def test_prompt_builder_no_gost(test_session: AsyncSession):
    ...
    assert "Simple Company" in system_content
...
@pytest.mark.asyncio
async def test_docx_builder(test_session: AsyncSession, company_with_all_fields: Company):
    ...
    try:
        result_path = docx_builder.build(result, company_with_all_fields, output_path)
...
async def test_docx_builder_gost(test_session: AsyncSession):
    """Test DocxBuilder with GOST enabled company."""
    company = Company(
        name="Gost Company",
        inn="7701123456",
        legal_form="ООО",
...
# ─── Test 14: Full API integration with influencing docs ──────────────────
...
@pytest.mark.asyncio
async def test_prompt_builder_empty_data(company_with_all_fields: Company):
